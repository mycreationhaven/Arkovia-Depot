(()=>{
  ...
  function signedIn(){return !!window.state?.session?.authenticated;}
  function renderMarket(){
    const grid=$('listingGrid'); if(!grid)return;
    const q=String($('marketSearch')?.value||'').trim().toLowerCase();
    const type=$('marketTypeFilter')?.value||'all';
    const sort=$('marketSort')?.value||'default';
    ...
    grid.innerHTML=list.map(l=>`<article class="card marketplace-card"><div class="market-card-top"><span class="asset-type-badge">${esc(l.assetType)}</span><span class="seller-name">by ${esc(l.sellerName)}</span></div><h3>${esc(l.assetName)}</h3><div class="price">${esc(l.price)} ${esc(window.state?.status?.currency||'ARKOS')}</div><div class="actions">${signedIn()?`<button class="buy-button" data-id="${esc(l.listingId)}">Buy now</button>`:'<button class="market-signin secondary" type="button">Sign in to buy</button>'}</div></article>`).join('');
    grid.querySelectorAll('.buy-button').forEach(b=>b.addEventListener('click',()=>window.buyListing?.(b.dataset.id,b)));
